...
contract MafagafoAvatarNft is MafagafoAvatarBase {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        Mafagafo memory _mafagafo = mafagafo[tokenId];
        string memory genes = toString(uint256(_mafagafo.genes));

        string memory __baseURI = _baseURI();
        return bytes(__baseURI).length > 0 ? string(abi.encodePacked(__baseURI, genes, ".json")) : "";
    }
